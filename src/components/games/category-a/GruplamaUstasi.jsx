...
const GruplamaUstasi = ({ onBack, colors, onGameComplete, prevBest }) => {
  ...
  const mkOpts=(cds)=>{const tot=cds.reduce((s,c)=>s+c.count,0);const o=[tot];let at=0,sp=3;while(o.length<4){if(at>30){sp++;at=0;}const c=Math.max(1,tot+Math.floor(Math.random()*sp*2)-sp);if(!o.includes(c))o.push(c);at++;}return o.sort(()=>Math.random()-0.5);};
