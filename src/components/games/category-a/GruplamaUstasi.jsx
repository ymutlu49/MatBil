...
const GruplamaUstasi = ({ onBack, colors, onGameComplete, rahatMod, prevBest }) => {
  ...
  const mkOpts=(cds)=>{
    const tot=cds.reduce((s,c)=>s+c.count,0);
    // Çeldiriciler toplamın büyüklüğüne göre ölçekli — küçük totallerde sıkışmasın
    const deltas = tot <= 5 ? [-1, 1, 2, -2, 3] : tot <= 10 ? [-2, 2, 1, -1, 3, -3] : [-3, 3, 2, -2, 5, -5];
    const o = [tot];
    for (const d of shuffle([...deltas])) {
      if (o.length >= 4) break;
      const v = Math.max(1, tot + d);
      if (!o.includes(v)) o.push(v);
    }
    // Garanti: 4'e ulaşmazsa artan filler
    let filler = 1;
    while (o.length < 4) {
      if (!o.includes(filler)) o.push(filler);
      filler++;
    }
    return shuffle(o);
  };
