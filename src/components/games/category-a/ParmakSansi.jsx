...
const ParmakSansi = ({ onBack, colors, onGameComplete, rahatMod, prevBest }) => {
  ...
  const Hand=({fingers,mirror=false})=>{
    ...
    const fingerData = [
      { x: 18, openH: 55, closedH: 22, w: 22, curve: -12, nail: true },  // başparmak
      { x: 38, openH: 75, closedH: 22, w: 19, curve: 0, nail: true },    // işaret
      { x: 58, openH: 82, closedH: 22, w: 18, curve: 0, nail: true },    // orta
      { x: 77, openH: 72, closedH: 22, w: 17, curve: 0, nail: true },    // yüzük
      { x: 94, openH: 58, closedH: 22, w: 16, curve: 8, nail: true },    // serçe
    ];

    return (
      <div className={`inline-block ${waveAnim ? 'anim-hand-wave' : ''}`}
        style={{
          filter: 'drop-shadow(0 4px 12px rgba(251, 191, 36, 0.4))',
        }}
      >
        <svg width="168" height="240" viewBox="0 0 130 185" style={{transform: mirror ? 'scaleX(-1)' : 'none'}}>
          {/* Avuç içi */}
  ...
  return (
    <div className={`h-screen ${colors?.bg} flex flex-col items-center p-3 overflow-hidden`}>
      ...
      <div className="bg-white p-4 rounded-2xl shadow-xl mb-3 flex gap-2 items-end justify-center min-h-[220px]">
